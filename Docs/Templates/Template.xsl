...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:b="http://www.fatturapa.gov.it/sdi/fatturapa/v1.1" xmlns:c="http://www.fatturapa.gov.it/sdi/fatturapa/v1.0" xmlns:a="http://ivaservizi.agenziaentrate.gov.it/docs/xsd/fatture/v1.2" xmlns:d="http://ivaservizi.agenziaentrate.gov.it/docs/xsd/fatture/v1.0">
	...
	<xsl:if test="a:FatturaElettronica">
        <xsl:variable name="TD">
            <xsl:value-of select="a:FatturaElettronica/FatturaElettronicaBody[1]/DatiGenerali/DatiGeneraliDocumento/TipoDocumento"/>
        </xsl:variable>
		<svg width="210mm" height="210mm" version="1.1" viewBox="0 0 210 210" xmlns="http://www.w3.org/2000/svg">
        <xsl:choose>
            <xsl:when test="$TD='TD01' or $TD='TD02'">
		   ...
		   <text font-family="Segoe UI" font-size="22" style="fill:white">
					<tspan x="13" y="130">
						<!--FORNITORE-->
						<xsl:value-of select="a:FatturaElettronica/FatturaElettronicaHeader/CedentePrestatore/DatiAnagrafici[1]/Anagrafica/Denominazione"/>
					</tspan>
					<tspan x="13" y="160">
						<!--CLIENTE-->
						<xsl:value-of select="a:FatturaElettronica/FatturaElettronicaHeader/CessionarioCommittente/DatiAnagrafici[1]/Anagrafica/Denominazione"/>
					</tspan>
					...
					<tspan x="13" y="70">
						<!--NUMERO FATTURA-->
						<xsl:value-of select="a:FatturaElettronica/FatturaElettronicaBody[1]/DatiGenerali/DatiGeneraliDocumento/Numero"/>
					</tspan>
					<tspan x="13" y="105" font-size="32" font-weight="bold">
						<!--DATA FATTURA-->
						<xsl:call-template name="FormatDateIta">
							<xsl:with-param name="DateTime" select="a:FatturaElettronica/FatturaElettronicaBody[1]/DatiGenerali/DatiGeneraliDocumento/Data"/>
						</xsl:call-template>
					</tspan>
					<tspan x="13" y="195" font-size="32"  font-weight="bold">
						<!--IMPORTO TOTALE-->
						<xsl:choose>
							<xsl:when test="a:FatturaElettronica/FatturaElettronicaBody[1]/DatiGenerali/DatiGeneraliDocumento/ImportoTotaleDocumento">
								<xsl:value-of select="format-number(a:FatturaElettronica/FatturaElettronicaBody[1]/DatiGenerali/DatiGeneraliDocumento/ImportoTotaleDocumento, '€ ###.###.##0,00', 'euro')"/>
							</xsl:when>
							<xsl:otherwise>
							<xsl:for-each select="a:FatturaElettronica/FatturaElettronicaBody[1]/DatiPagamento">
